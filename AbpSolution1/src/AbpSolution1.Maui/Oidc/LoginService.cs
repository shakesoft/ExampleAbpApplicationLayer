...
public class LoginService : ILoginService, ITransientDependency
{
    ...
    {
        var token = await _storage.GetAsync(AbpSolution1Consts.OidcConsts.AccessTokenKeyName);
    ...
    {
        var refreshToken = await _storage.GetAsync(AbpSolution1Consts.OidcConsts.RefreshTokenKeyName);
    ...
    {
        await _storage.SetAsync(AbpSolution1Consts.OidcConsts.AccessTokenKeyName, accessToken);
        await _storage.SetAsync(AbpSolution1Consts.OidcConsts.RefreshTokenKeyName, refreshToken);
    }

    private async Task ClearTokenCacheAsync()
    {
        await _storage.RemoveAsync(AbpSolution1Consts.OidcConsts.AccessTokenKeyName);
        await _storage.RemoveAsync(AbpSolution1Consts.OidcConsts.RefreshTokenKeyName);
    }
}