...
public interface IAuditLogsAppService : IApplicationService
{
    ...
    Task<ExportAuditLogsOutput> ExportToExcelAsync(ExportAuditLogsInput input);
    
    Task<IRemoteStreamContent> DownloadExcelAsync(string fileName);

    Task<ExportEntityChangesOutput> ExportEntityChangesToExcelAsync(ExportEntityChangesInput input);
}
