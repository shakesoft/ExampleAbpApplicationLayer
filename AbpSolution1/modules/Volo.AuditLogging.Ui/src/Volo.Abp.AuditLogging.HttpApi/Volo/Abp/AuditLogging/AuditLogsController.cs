...
public class AuditLogsController : AbpControllerBase, IAuditLogsAppService
{
    ...
    {
        return await AuditLogsAppService.ExportToExcelAsync(input);
    }

    [HttpGet]
    [Route("download-excel/{fileName}")]
    public virtual async Task<IRemoteStreamContent> DownloadExcelAsync(string fileName)
    {
        return await AuditLogsAppService.DownloadExcelAsync(fileName);
    }

    [HttpGet]
    [Route("export-entity-changes-to-excel")]
    public virtual async Task<ExportEntityChangesOutput> ExportEntityChangesToExcelAsync(ExportEntityChangesInput input)
    {
        return await AuditLogsAppService.ExportEntityChangesToExcelAsync(input);
    }
}
