...
public abstract class AbpAccountImpersonationChallengeAccountController : ChallengeAccountController
{
            ...
            var properties = new AuthenticationProperties(new Dictionary<string, string>()
                {
                    ...
                    { AbpAccountImpersonationConsts.ReturnUrl, configuration["App:SelfUrl"] + (returnUrl.IsNullOrWhiteSpace() ? "" : returnUrl.EnsureStartsWith('/')) }
                })
            {
                RedirectUri = "/"
            };
    ...
    public virtual Task<IActionResult> BackToImpersonatorAsync()
    {
        if (CurrentUser.IsAuthenticated)
        {
            var properties = new AuthenticationProperties(new Dictionary<string, string>()
                {
                    { AbpAccountImpersonationConsts.BackToImpersonator, AbpAccountImpersonationConsts.BackToImpersonator }
                })
            {
                RedirectUri = "/"
            };
