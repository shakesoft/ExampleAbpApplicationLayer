                ...
                using (CurrentPrincipalAccessor.Change(authenticateResult.Principal))
                {
                    ...
                    using (CurrentTenant.Change(TenantId))
                    {
                        ...
                        if (adminUser != null)
                        {
                            try
                            {
                                await TryToSwitchUserDuringImpersonateAsync(oldUser, adminUser, oldUserIsBeingImpersonated);

                                return await OpenIddictAuthorizeResponse.GenerateAuthorizeResponseAsync(HttpContext,
                                    adminUser,
                                    new[]
                                    {
